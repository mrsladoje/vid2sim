...
- **Video diffusion on the critical path** (Wan 2.5 VACE, CogVideoX-Fun-Control, etc.).
...
- **Gaussian splats.** Any GS pipeline would need a custom collider path to integrate with Rapier/PyBullet (ADR-004), and the browser demo (ADR-006) loads glTF meshes, not splats. The cost of building a splat-to-mesh or splat-collider bridge exceeds the benefit.
...
- **Video diffusion (Wan 2.5 VACE, CogVideoX-Fun-Control) on the critical path.** Costs 30–90 s per second of output — overnight work at best. Cannot be inside the 90 s per-scene reconstruction budget (PRD §3 goal 2).
...
- PRD §13 (Risks)
- Related: ADR-003 (chose mesh diffusion over splats), ADR-004 (chose Rapier+PyBullet over Isaac/Genesis/MuJoCo runtime), ADR-007 (no cloud compute either).
